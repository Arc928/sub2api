    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          ...
          >
            <!-- Header -->
            <div class="relative overflow-hidden border-b border-hairline bg-canvas px-6 py-5 dark:border-dark-600 dark:bg-dark-800">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  <div class="flex items-center gap-2">
                    <div class="flex h-8 w-8 items-center justify-center rounded-sm border border-hairline text-ink dark:border-dark-500 dark:text-gray-100">
                  ...
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <span class="font-medium text-ink dark:text-gray-100">{{ unreadCount }}</span>
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    @click="markAllAsRead"
                    :disabled="loading"
                    class="rounded-sm bg-ink px-4 py-2 text-xs font-medium text-canvas transition-colors hover:bg-charcoal disabled:opacity-50 dark:bg-gray-100 dark:text-ink dark:hover:bg-white"
                  >
                    {{ t('announcements.markAllRead') }}
                  </button>
                  <button
                    @click="closeModal"
                    class="flex h-9 w-9 items-center justify-center rounded-sm text-gray-500 transition-colors hover:bg-surface-soft hover:text-gray-700 dark:text-gray-400 dark:hover:bg-dark-700 dark:hover:text-gray-300"
              ...
              <div v-else-if="announcements.length > 0">
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    <div
                      v-if="!item.read_at"
                      class="relative flex h-10 w-10 items-center justify-center rounded-sm bg-ink text-canvas dark:bg-gray-100 dark:text-ink"
                    >
                      <!-- Pulse ring -->
                      <span class="absolute inline-flex h-full w-full animate-ping rounded-sm bg-current opacity-20"></span>
                      <!-- Icon -->
                  ...
                  <div
                    v-if="!item.read_at"
                    class="absolute left-0 top-0 h-full w-1 bg-ink dark:bg-gray-100"
                  ></div>
                </div>
              </div>
          ...
          >
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mb-3 flex items-center gap-2">
                    <div class="flex h-10 w-10 items-center justify-center rounded-sm border border-hairline text-ink dark:border-dark-500 dark:text-gray-100">
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="rounded-sm bg-surface-card px-2.5 py-1 text-xs font-medium text-ink dark:bg-dark-700 dark:text-gray-100">
                        {{ t('announcements.title') }}
                      </span>
                      <span
                        v-if="!selectedAnnouncement.read_at"
                        class="inline-flex items-center gap-1.5 rounded-sm bg-ink px-2.5 py-1 text-xs font-medium text-canvas dark:bg-gray-100 dark:text-ink"
                      >
            ...
            <div class="max-h-[60vh] overflow-y-auto bg-white px-8 py-8 dark:bg-dark-800">
              ...
              <div class="relative">
                ...
                <div class="absolute left-0 top-0 bottom-0 w-1 bg-hairline-strong"></div>
            ...
            <div class="border-t border-gray-100 bg-gray-50/50 px-8 py-5 dark:border-dark-700 dark:bg-dark-900/30">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  <button
                    @click="closeDetail"
                    class="rounded-sm border border-hairline-strong bg-canvas px-5 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-surface-soft dark:border-dark-500 dark:bg-dark-700 dark:text-gray-300 dark:hover:bg-dark-600"
                  >
                    {{ t('common.close') }}
                  </button>
                  <button
                    ...
                    class="rounded-sm bg-ink px-5 py-2.5 text-sm font-medium text-canvas transition-colors hover:bg-charcoal active:bg-ink-deep dark:bg-gray-100 dark:text-ink dark:hover:bg-white"
                  >
